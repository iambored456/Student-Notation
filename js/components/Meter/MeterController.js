// js/components/Meter/MeterController.js
...
class MeterController {
  ...
  async createMeterFromExistingMic() {
    ...
    const PitchPaintService = (await import('../../services/pitchPaintService.js')).default;
  ...
  stopMeter() {
    ...
    this.updateUI();
    console.log('MeterController: NexusUI Meter stopped');
  }
  ...
  handleFpsChange(newFps) {
    ...
    // Note: NexusUI Meter handles animation internally
    // We keep the setting for potential future use or custom implementations
  }
  ...
  async handleAutoCalibrate() {
    ...
    try {
      ...
      // NexusUI Meter doesn't expose internal dB values
  ...
  handleReset() {
    ...
    if (this.meter) {
      const dimensions = this.getMeterDimensions();
      this.meter.resize(dimensions[0], dimensions[1]);
      // Note: NexusUI Meter doesn't have setFramerate method
      // Frame rate is handled internally by NexusUI
    }
  }
