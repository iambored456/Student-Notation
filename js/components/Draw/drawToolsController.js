...
class DrawToolsController {
    ...
    initialize() {
        ...
        this.optionsContainer = document.getElementById('draw-tool-options');

        if (!this.toolButtons.length || !this.optionsContainer) {
            console.warn('[DrawTools] Could not find draw tool elements');
            return;
        }
    ...
    selectTool(toolName) {
        ...
        this.renderToolOptions(toolName);
    ...
    /**
     * Render options UI for the selected tool
     ...
     */
    renderArrowOptions() {
        // Create compact button bar with popup menus
        this.optionsContainer.innerHTML = `
    ...
    attachArrowToolbarListeners() {
        const buttons = this.optionsContainer.querySelectorAll('[data-popup]');
        ...
        // Swap button
        const swapButton = this.optionsContainer.querySelector('[data-action="swap-endpoints"]');
        if (swapButton) {
    ...
    renderTextOptions() {
        ...
        this.attachCommonToolbarListeners();

        // Decrease font size button
        const decreaseButton = this.optionsContainer.querySelector('[data-action="decrease-size"]');
        if (decreaseButton) {
        ...
        const increaseButton = this.optionsContainer.querySelector('[data-action="increase-size"]');
        if (increaseButton) {
        ...
        const bgButton = this.optionsContainer.querySelector('[data-action="toggle-background"]');
        if (bgButton) {
        ...
        const superscriptButton = this.optionsContainer.querySelector('[data-action="toggle-superscript"]');
        ...
        const subscriptButton = this.optionsContainer.querySelector('[data-action="toggle-subscript"]');
        if (subscriptButton) {
            // Prevent blur on mousedown to keep text input focused
    ...
    /**
     * Attach common toolbar listeners for text, marker, highlighter tools
     */
