// js/components/Meter/Meter.js — standalone ES module for audio level metering (RMS, multi-channel)
  ...
  render(nowTime = performance.now()) {
    ...
    if (this.active && typeof performance !== 'undefined') {
      ...
      if (frameTime > 16 && Math.random() < 0.01) { // Sample 1% of frames
        console.warn(`Meter: Slow frame detected: ${frameTime.toFixed(2)}ms`);
      }
    } else {
      this._drawFrame();
    }
