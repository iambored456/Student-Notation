...
function debugNotePlacement(stage: string, details: Record<string, unknown> = {}): void {
...
function findMeasureSnapPoint(columnIndex) {
  const { macrobeatGroupings, macrobeatBoundaryStyles, musicalColumnWidths } = store.state;
  // columnIndex is canvas-space (0 = first musical beat)
  // Use musicalColumnWidths to get the count of musical columns
  const musicalColumns = (musicalColumnWidths || []).length;
...
function handleMouseMove(e) {
  ...
  const musicalColumnsLength = store.state.musicalColumnWidths?.length || store.state.columnWidths.length - 4;
