...
export function drawLegends(ctx: CanvasRenderingContext2D, options: LegendOptions, startRow: number, endRow: number): void {
  ...
  let leftScale: string[] = [];
  let rightScale: string[] = [];
  ...
  function drawLegendColumn(startCol: number, columnsOrder: readonly ('A' | 'B')[]): void {
    ...
    const colWidthsPx = columnWidths.slice(startCol, startCol + 2).map((w: number) => w * cellWidth);
      ...
      for (let rowIndex = startRow; rowIndex <= endRow; rowIndex++) {
        ...
        if (row.column === colLabel) {
          ...
          if (shouldApplyTransparency) {
            ...
            const rgbMatch = /^#([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(bgColor);
  ...
  drawLegendColumn(columnWidths.length - 2, ['A', 'B'] as const);
}
...
export function drawLegendsToSeparateCanvases(
  ...
  const processLabel = (
    ...
    if (!rowData) {
      const noteMatch = label.match(/^([A-G][♭♯#b]*)(\d+)$/);
      if (!noteMatch) {return label;}
      const noteName = noteMatch[1] ?? '';
      const octave = noteMatch[2] ?? '';
      const sharpName = noteName.replace(/♭/g, '').replace(/♯/, '#') || noteName;
      const flatName = noteName.replace(/♯/g, '').replace(/#/g, 'b') || noteName;

      if (sharp && flat) {return `${flatName}/${sharpName}${octave}`;}
      else if (sharp && !flat) {return `${sharpName}${octave}`;}
      else if (flat && !sharp) {return `${flatName}${octave}`;}
      else {return `${sharpName}${octave}`;}
    }

    const sharpName = rowData.sharpName || rowData.pitch;
    const flatName = rowData.flatName || rowData.pitch;
    const sharpNormalized = sharpName.replace(/♯/g, '#').replace(/\d+$/, '');
    const flatNormalized = flatName.replace(/♭/g, 'b').replace(/\d+$/, '');
    ...
    let result;
    if (sharp && flat) {result = rowData.pitch;}
    else if (sharp && !flat) {result = sharpName;}
    else if (flat && !sharp) {result = flatName;}
    else {result = sharpName;}
  ...
  function drawSingleLegend(
    ...
    // Calculate column widths for this legend (2 columns)
    const colWidthsPx = columnWidths.slice(startCol, startCol + 2).map((w: number) => w * cellWidth);
    ...
    columnsOrder.forEach((colLabel: 'A' | 'B', colIndex: number) => {
      ...
      for (let rowIndex = startRow; rowIndex <= endRow; rowIndex++) {
        ...
        if (row.column === colLabel) {
          ...
          const textAlpha = shouldApplyTransparency ? '00' : 'FF';
  ...
  if (leftCtx) {
    drawSingleLegend(leftCtx, 0, ['B', 'A'] as const, leftScale);
  }

  // Draw right legend if context is available
  if (rightCtx) {
    drawSingleLegend(rightCtx, columnWidths.length - 2, ['A', 'B'] as const, rightScale);
  }
}
