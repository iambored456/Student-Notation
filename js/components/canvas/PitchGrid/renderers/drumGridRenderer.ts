...
import type { PlacedNote, TonicSign, MacrobeatBoundaryStyle } from '../../../../../types/state.js';

type DrumPlacedNote = PlacedNote & { isDrum?: boolean; drumTrack?: string | null };

interface DrumRendererOptions {
  placedNotes: DrumPlacedNote[];
  columnWidths: number[];
  cellWidth: number;
  cellHeight: number;
...
function getColumnX(index: number): number {
...
function drawVerticalGridLines(ctx: CanvasRenderingContext2D, options: DrumRendererOptions): void {
 ...
 */
export function drawDrumGrid(ctx: CanvasRenderingContext2D, options: DrumRendererOptions): void {
  const { placedNotes, columnWidths, cellWidth, cellHeight, placedTonicSigns } = options;
  const drumNotes = placedNotes;
  ...
  for (let col = 2; col < totalColumns - 2; col++) {
    ...
    const currentCellWidth = (columnWidths[col] ?? 0) * cellWidth;

    for (let row = 0; row < 3; row++) {
      const y = row * drumRowHeight;
      const drumTrack = drumLabels[row];

      const drumHit = drumNotes.find(note =>
