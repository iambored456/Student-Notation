...
let masterGain: Tone.Gain | null = null; // Master gain with polyphony-aware scaling
let volumeControl: Tone.Volume | null = null; // User-controllable volume (independent of auto-gain)
let compressor: Tone.Compressor | null = null; // Bus compressor for gentle leveling
let limiter: Tone.Limiter | null = null; // Final safety limiter
let clippingMeter: Tone.Meter | null = null; // Meter for detecting near-clipping levels
let waveformAnalyzers: Record<string, Tone.Analyser> = {}; // Store analyzers for waveform visualization
let activeVoiceCount = 0; // Track total active voices across all synths (instantaneous)
let smoothedVoiceCount: number = POLYPHONY_REFERENCE; // Initialize to reference to avoid initial boost
let gainUpdateLoopId: NodeJS.Timeout | null = null; // ID for scheduled gain updates
let clippingMonitorId: NodeJS.Timeout | null = null;
let lastClippingWarningAt = 0;

// Debug helper: set true to log voice counts during drag previews
const DEBUG_PREVIEW_AUDIO = true;

function logVoiceState(action: string, color: string, synth: any) {
  if (!DEBUG_PREVIEW_AUDIO || !synth) {return;}
  const activeVoices = (synth as any)._activeVoices?.size ?? (synth as any)._voices?.length ?? 'n/a';
}
...
function startClippingMonitor() {
  ...
  clippingMonitorId = setInterval(() => {
    ...
    if (levelValue !== undefined && levelValue > CLIPPING_WARNING_THRESHOLD_DB) {
      const now = Date.now();
      if (now - lastClippingWarningAt >= CLIPPING_WARNING_COOLDOWN_MS) {
        lastClippingWarningAt = now;
        logger.warn('SynthEngine', 'Limiter input approaching clipping threshold', { level: levelValue }, 'audio');
      }
    }
  }, CLIPPING_MONITOR_INTERVAL_MS);
}

// Custom options interface for FilteredVoice
interface FilteredVoiceOptions extends Tone.SynthOptions {
  filter?: any;
...
// Extended PolySynth type with custom properties (using type instead of interface to avoid extension issues)
type ExtendedPolySynth = Tone.PolySynth & {
  _currentVibrato?: { speed: number; span: number };
  _currentTremolo?: { speed: number; span: number };
  _currentFilter?: any;
  _activeVoices?: Map<number, any>;
  _voices?: any[];
};
...
const SynthEngine = {
  init() {
    ...
    for (const color in store.state.timbres) {
      ...
      const synth = new Tone.PolySynth({
        maxPolyphony: 48, // Cap voices to prevent runaway voice buildup while allowing larger chords/previews
        voice: FilteredVoice,
        options: {
    ...
    store.on('volumeChanged', (dB: number) => this.setVolume(dB));
  ...
  setBpm(tempo: number) {
    try {
      if (Tone?.Transport?.bpm) {
        ...
        logger.debug('SynthEngine', `Tone.Transport BPM updated to ${tempo}`, null, 'audio');
      }
    } catch (error) {
      logger.warn('SynthEngine', 'Unable to update BPM on Tone.Transport', { tempo, error }, 'audio');
    }
  },
  ...
  triggerAttack(pitch: string | number, color: string, time = Tone.now(), isDrum = false) {
    const synth = synths[color];
    if (synth) {
      const maxPoly = (synth as any).maxPolyphony ?? 48;
      const synthVoices = (synth as any)._activeVoices?.size ?? (synth as any)._voices?.length ?? 0;

      // If Tone thinks we're at/over the cap, force a reset to avoid the max-polyphony drop spam
      if (synthVoices >= maxPoly - 1) {
        logVoiceState('attack-pre-reset', color, synth);
        synth.releaseAll();
        activeVoiceCount = 0;
      }
      ...
      activeVoiceCount++;
      logVoiceState('attack++', color, synth);

      // Warn when nearing max poly but don't skip attacks (Tone will steal voices as needed)
      const currentVoices = (synth as any)._activeVoices?.size ?? (synth as any)._voices?.length ?? 0;
      if (currentVoices >= maxPoly - 4) {
        logVoiceState('attack-near-maxpoly', color, synth);
      }
      ...
      if (isDrum && window.getDrumVolume) {
        ...
        setTimeout(() => {
          if (synth?.volume) {synth.volume.value = originalVolume;}
        }, 100);
      } else {
        synth.triggerAttack(pitch, time);
      }
  ...
  /**
   * Force a near-instant release for specific preview pitches by temporarily
   * zeroing the envelope release, then restoring the original value.
   */
  quickReleasePitches(pitches: Array<string | number>, color: string) {
    ...
    try {
      ...
      logVoiceState('quickRelease:before', color, synth);
      pitches.forEach(pitch => synth.triggerRelease(pitch, Tone.now()));
      logVoiceState('quickRelease:after', color, synth);

      // Clamp active voice count to current synth voices after quick release
      const currentVoices = (synth as any)._activeVoices?.size ?? (synth as any)._voices?.length ?? activeVoiceCount;
      activeVoiceCount = Math.max(0, Math.min(activeVoiceCount, currentVoices));

      // If voices are still piling up (e.g., effects tails keeping voices alive), hard reset for previews
      const maxPoly = (synth as any).maxPolyphony ?? 48;
      if (currentVoices > maxPoly * 0.75) {
        logVoiceState('quickRelease:hard-reset', color, synth);
        synth.releaseAll();
        activeVoiceCount = 0;
      }
      logVoiceState('quickRelease:clamped', color, synth);
    } catch (err) {
      logger.warn('SynthEngine', 'quickReleasePitches failed', { err, color, pitches }, 'audio');
    } finally {
  ...
  triggerRelease(pitch: string | number, color: string, time = Tone.now()) {
    ...
    if (synth) {
      ...
      const currentVoices = (synth as any)._activeVoices?.size ?? (synth as any)._voices?.length ?? activeVoiceCount;
      activeVoiceCount = Math.max(0, Math.min(activeVoiceCount, currentVoices));
      logVoiceState('release--', color, synth);
    }
  },
  ...
  /**
     ...
     */
  getAllSynths(): Record<string, any> {
    return { ...synths };
  },
