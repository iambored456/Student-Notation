...
const RhythmService = {
  ...
  getTimeSignatureSegments(): TimeSignatureSegment[] {
    ...
    macrobeatGroupings.forEach((groupValue, index) => {
      ...
      if (isSolidBoundary || isLastBeat) {
        ...
        measureStartX = getColumnX(measureStartColumn, renderOptions as any);
        measureEndX = getColumnX(measureEndColumn, renderOptions as any);
  ...
  getRhythmUIButtons(): RhythmUIButton[] {
    ...
    macrobeatGroupings.forEach((group, index) => {
      ...
      // Ensure musicalColumnWidths is always available (fallback to columnWidths.slice(2, -2))
      const canvasSpaceRenderOptions = {
        ...state,
        modulationMarkers: state.modulationMarkers || [],
        cellWidth: state.cellWidth,
        columnWidths: state.columnWidths,
        musicalColumnWidths: state.musicalColumnWidths || state.columnWidths.slice(2, -2),
        baseMicrobeatPx: state.cellWidth
      };
