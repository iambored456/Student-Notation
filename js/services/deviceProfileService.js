// js/services/deviceProfileService.js
...
function computeDeviceProfile() {
    ...
    const orientation = orientationQuery
    ...
    const touchPoints = navigatorObj?.maxTouchPoints ?? navigatorObj?.msMaxTouchPoints ?? 0;
...
function applyCssFlags(profile) {
...
export function initDeviceProfileService(options = {}) {
    const win = safeWindow();
    if (!win || initialized) {
        return;
    }
...
export function onDeviceProfileChange(callback) {
    if (typeof callback !== 'function') {
        return () => {};
    }
    store.on('deviceProfileChanged', callback);
