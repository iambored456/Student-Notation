...
let /* gridContainer, */ pitchGridWrapper: HTMLElement | null,
  ...
  pitchPaintCanvas: HTMLCanvasElement | null,
...
function initDOMElements() {
  ...
  pitchPaintCanvas = document.getElementById('pitch-paint-canvas') as HTMLCanvasElement | null;
...
function recalcAndApplyLayout() {
  ...
  // Only fall back to total width if no modulation present
  const hasModulation = store.state.modulationMarkers && store.state.modulationMarkers.length > 0;
  const finalCanvasWidth = hasModulation ? modulatedCanvasWidth : totalCanvasWidth;
  ...
  if (hasModulation) {
    // Use modulated width calculation
    const musicalColumnWidths = store.state.musicalColumnWidths || [];
    const renderOptions = {
      ...store.state,
    ...
    // Get modulated width of all musical columns (canvas-space: 0 to length)
    middleCellWidth = getColumnX(musicalColumnWidths.length, renderOptions);
  } else {
  ...
  if (buttonGridWrapper) {
    ...
    const leftCellWidth = store.state.columnWidths.slice(0, 2).reduce((sum: number, w: number) => sum + w * store.state.cellWidth, 0);
    ...
    const rightCellWidth = store.state.columnWidths.slice(-2).reduce((sum: number, w: number) => sum + w * store.state.cellWidth, 0);
  ...
  // - Main canvases (notation-grid, playhead, hover, pitch-paint):
  ...
  const leftLegendWidth = ((newColumnWidths[0] || 0) + (newColumnWidths[1] || 0)) * store.state.cellWidth;
  const rightLegendWidth = ((newColumnWidths[newColumnWidths.length - 2] || 0) + (newColumnWidths[newColumnWidths.length - 1] || 0)) * store.state.cellWidth;

  // Musical canvas width = total width - left legend - right legend
  const musicalCanvasWidthPx = Math.round(totalCanvasWidthPx - leftLegendWidth - rightLegendWidth);
  const leftLegendWidthPx = Math.round(leftLegendWidth);
  const rightLegendWidthPx = Math.round(rightLegendWidth);
...
const LayoutService = {
  ...
  init() {
    ...
    const { ctx, drumCtx, legendLeftCtx, legendRightCtx, canvasContainer } = initDOMElements();
  ...
  zoomOut() {
    ...
    if (!store.state.isPitchRangeLocked) {
      ...
      if (canExpandRange) {
        const oldRowCount = currentRange.bottomIndex - currentRange.topIndex + 1;
  ...
  scrollByPixels(deltaY: number, deltaX = 0) {
  ...
  getModulatedCanvasWidth() {
    ...
    try {
      ...
      const columnWidths = store.state.columnWidths || [];  // Full-space for layout
      const musicalColumnWidths = store.state.musicalColumnWidths || [];  // Canvas-space
      ...
      // Get modulated width of ALL musical columns (canvas-space: 0 to length)
      const modulatedMusicalWidth = getColumnX(musicalColumnWidths.length, renderOptions);

      // Calculate legend widths (still from full columnWidths for layout)
      const leftLegendWidth = ((columnWidths[0] || 0) + (columnWidths[1] || 0)) * cellWidth;
      const rightLegendWidth = ((columnWidths[columnWidths.length - 2] || 0) +
                                (columnWidths[columnWidths.length - 1] || 0)) * cellWidth;

      const finalWidth = leftLegendWidth + modulatedMusicalWidth + rightLegendWidth;
