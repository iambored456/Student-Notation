...
function calculateTimeMap() {
  ...
  // state.columnWidths is currently in FULL-SPACE (includes legend columns)
  // We need to extract CANVAS-SPACE widths (musical columns only, indices 2 to length-2)
  const musicalColumnWidths = columnWidths.slice(2, -2);
...
function scheduleNotes() {
  ...
  store.state.placedNotes.forEach((note, noteIndex) => {
    ...
    if (note.isDrum) {
      ...
      const loopDuration = configuredLoopEnd - configuredLoopStart;
      ...
      if (needsClamp) {
        const originalReleaseTime = releaseTime;
...
const TransportService = {
  ...
  start() {
    ...
    void audioInit().then(() => {
      ...
      if (window.PaintPlaybackService?.onTransportStart) {
  ...
  stop() {
    ...
    if (window.PaintPlaybackService?.onTransportStop) {
