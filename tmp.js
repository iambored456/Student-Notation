...
class AdsrComponent {
    ...
    resize() {
        ...
        const graphWrapper = this.ui.container.closest('.adsr-graph-wrapper');
        const controlsGrid = graphWrapper?.closest('.adsr-controls-grid');
        const adsrContainer = controlsGrid?.closest('.adsr-container');
        const sectionPanel = adsrContainer?.closest('.timbre-section-panel');
    ...
    render() {
        if (!this.ui || !this.width || !this.height) {
            return;
        }
