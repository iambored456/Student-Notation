...
import type { PlacedNote, Store, CanvasSpaceColumn } from '../../../types/state.js';
...
function updateGlobalRow(note: PlacedNote, state: Store['state']): void {
  ...
  const topIndex = state.pitchRange?.topIndex ?? 0;
  note.globalRow = note.row + topIndex;
}
...
export const noteActions = {
  ...
  addTonicSignGroup(this: Store, tonicSignGroup: { preMacrobeatIndex: number; columnIndex: CanvasSpaceColumn; row: number; tonicNumber: number }[]): void {
    console.log('[noteActions] addTonicSignGroup CALLED with:', tonicSignGroup);
    logger.debug('TonicPlacement', 'Starting addTonicSignGroup', { tonicSignGroup }, 'state');

    const firstSign = tonicSignGroup[0];
    if (!firstSign) {
      console.log('[noteActions] EARLY RETURN: firstSign is falsy');
      return;
    }
    const { preMacrobeatIndex } = firstSign;
    console.log('[noteActions] preMacrobeatIndex:', preMacrobeatIndex);
    ...
    if (existingEntry) {
      console.log('[noteActions] EARLY RETURN: Existing tonic already present for measure', preMacrobeatIndex);
      logger.debug('TonicPlacement', 'Existing tonic already present for measure, skipping', {
        preMacrobeatIndex
      }, 'state');
      return;
    }
    console.log('[noteActions] No existing tonic found, proceeding to add new one');
    ...
    console.log('[noteActions] Emitting rhythmStructureChanged event after placing tonic');
    this.emit('notesChanged');
