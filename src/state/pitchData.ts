 ...
 *
 * STRUCTURE:
 * Contains 107 pitch rows from C♯8/D♭8 (highest) to A♭0/G♯0 (lowest).
 ...
 *
 * BOUNDARY ROWS:
 * - Row 0 (C♯8/D♭8): Top boundary row - provides visual space for the complete top half of C8
 * - Row 106 (A♭0/G♯0): Bottom boundary row - provides visual space for the complete bottom half of A0
 * - Both boundary rows have white backgrounds (#ffffff) and their text labels are hidden in the legend renderer
 * - This allows users to see the complete top/bottom half-cells without expanding the canvas
 *
 * MUSICAL RANGE:
 * - Audible range: Rows 1-105 (C8 to A0) - standard 88-key piano range
 * - Boundary rows are visual-only and not intended for note placement
 */
export const fullRowData: PitchRowData[] = [
  // === BOUNDARY ROW (TOP) ===
  // Provides visual padding so C8's top half-cell is fully visible
  { pitch: 'C♯/D♭8', flatName: 'D♭8', sharpName: 'C♯8', toneNote: 'Db8', frequency: 4434.92, column: 'B', hex: '#fcfcfc', isAccidental: true },
  ...
  // === BOUNDARY ROW (BOTTOM) ===
  // Provides visual padding so A0's bottom half-cell is fully visible
  { pitch: 'A♭/G♯0', flatName: 'A♭0', sharpName: 'G♯0', toneNote: 'Ab0', frequency: 25.96, column: 'A', hex: '#242c48', isAccidental: true }
];
