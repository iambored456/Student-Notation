...
class BaseAnimationEffect {
  ...
  initBase() {
    ...
    store.on('visualEffectChanged', ({ effectType, parameter: _parameter, value: _value, color, effectParams }) => {
    ...
    store.on('spacebarPlayback', ({ color: _color, isPlaying }) => {
      if (isPlaying) {
        this.isPlaybackActive = true;
      } else {
        this.isPlaybackActive = false;
  ...
  /**
     ...
     */
  updateAnimationParameters(_color, _effectParams) {
  ...
  /**
     ...
     */
  updateAnimationPhases(_currentTime) {
