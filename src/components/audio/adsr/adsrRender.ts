...
export function drawEnvelope(envelopeLayer: SVGGElement, nodeLayer: SVGGElement, points: Point[], { height, width }: Dimensions, colorKey: string, maxTime: number, canvasCtx: CanvasRenderingContext2D | null): void {
  ...
  let reverbParams: EffectParams = { decay: 0, roomSize: 0 };
  ...
  const drawReverbShadow = (envelopePoints: Point[], opacityMultiplier = 1.0): void => {
    if (!canvasCtx || reverbParams.decay === 0 || reverbParams.roomSize === 0 || !maxTime) {return;}
    const sustainPoint = envelopePoints[1];
    const releasePoint = envelopePoints[3];
    if (!sustainPoint || !releasePoint) {return;}

    const shadowLength = ((reverbParams.decay || 0) / 100) * 5 * (width / maxTime); // Expanded from 3 to 5 seconds
    const shadowDarkness = ((reverbParams.roomSize || 0) / 100) * opacityMultiplier;
    ...
    for (let i = 0; i < numSamples; i++) {
      ...
      const edgeY = sustainPoint.y + (releasePoint.y - sustainPoint.y) * t;
      const nextEdgeY = sustainPoint.y + (releasePoint.y - sustainPoint.y) * nextT;
      const edgeX = sustainPoint.x + (releasePoint.x - sustainPoint.x) * t;
      const nextEdgeX = sustainPoint.x + (releasePoint.x - sustainPoint.x) * nextT;
  ...
  const releaseOpacity = (reverbParams.decay || 0) > 0 && (reverbParams.roomSize || 0) > 0
    ? Math.max(0.3, 1 - (((reverbParams.decay || 0) / 100) * ((reverbParams.roomSize || 0) / 100) * 0.7))
    : 1.0;
