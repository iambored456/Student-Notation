...
let _filteredCoeffs = new Float32Array(BINS).fill(0);
...
function handleBinPointerEvent(e, binIndex = null) {
  ...
  if (clampedValue === 0) {
    ...
    if (window.staticWaveformVisualizer?.generateWaveform) {
...
export function initHarmonicBins() {
  ...
  for (let i = 0; i < BINS; i++) {
    ...
    phaseBtn.addEventListener('pointerenter', () => {
      if (isDraggingBin) {
        recordHarmonicBinsDebug('log', '[SNAP-TO-ZERO] ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â¯ Drag entered phase button for bin', i + 1);
        ...
        setTimeout(() => {
          ...
          updateSliderVisuals();
          recordHarmonicBinsDebug('log', '[SNAP-TO-ZERO] ÃƒÂ¢Ã…â€œÃ¢â‚¬Å“ Bin', i + 1, 'snapped to ZERO via phase button drag');
        }, rampTime * 1000);
      }
    });
    ...
    phaseBtn.addEventListener('click', (e) => {
      ...
      if (window.staticWaveformVisualizer?.startPhaseTransition) {
      ...
      logger.debug('HarmonicBins', `Phase button clicked for H${i + 1}, new phase: ${degrees}Ãƒâ€šÃ‚Â°`, null, 'filter');
    });
  ...
  store.on('timbreChanged', (color) => {
    if (color === currentColor) {
      ...
      validateStateSync(coeffs || new Float32Array(12), phases || new Float32Array(12), timbre, 'timbreChanged-before');
